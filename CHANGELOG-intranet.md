...
- `w0-03-test-auth-baseline`：测试鉴权基线（只改测试侧）。`bootstrap_admin` 显式授予全部权限键（运行时取自 `ALL_PERMISSION_KEYS`）；新增 `env_admins` 夹具与 `AdminCredentials`，预留系统 / 安全 / 审计三个管理员槽位（`w3-03` 之前都指向同一个管理员），夹具内的建账与查账走安全管理员槽位；新增只在测试进程内生效的 `tests/support/auth_guards.py`（`CAPTCHA_SEAMS`、`DEPENDENCY_OVERRIDES`、`REAL_AUTH_GUARD_MODULES`、`real_auth_guards` 标记），测试默认豁免登录验证码，验证码专项测试退出豁免；既有测试不改。
