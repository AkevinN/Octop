...
- [ ] 3. 验证码测试豁免与生产隔离不变式（测试先行，0.75 人日）
  - [ ] 3.1 先写会失败的测试
...
- [ ] 5. 为三员分立预留管理员凭据形状（测试先行，0.75 人日）
  ...
  - [ ] 5.2 实现 `tests/support/auth.py` 的形状
    - 改动：`tests/support/auth.py`：
