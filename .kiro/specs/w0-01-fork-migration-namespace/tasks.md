...
- [ ] 3. 事务化 runner（测试先行，0.75 人日）
  - [ ] 3.1 先写会失败的 runner 用例
  ...
  - [ ] 3.2 实现 `run_fork_migrations`
    - 改动：`src/octop/infra/db/fork_migrate.py` 的 `run_fork_migrations` 按以下顺序实现：
      1. 建表并写初始行；
      2. 发现文件，计算最大版本；
      3. 程序回退时记 WARNING 并返回；
      4. 逐版本在 `db.transaction()` 内完成：重读水位（PG 加 `FOR UPDATE`）→ 用 `migrate._split_pg_sql` 切分后逐条执行 → 调用可选的 Python 步骤 → `UPDATE _fork_schema_version SET version = ? WHERE id = 1`；
      5. 提交后记 INFO 日志（含文件名）。
    - 验证：`uv run pytest tests/unit/db/test_fork_migrate.py -q`
    - 验证：`uv run mypy src/octop/infra/db/fork_migrate.py`
    - _需求：2.1, 2.2, 2.3, 2.4, 2.5, 2.6_

- [ ] 4. 接入 `run_migrations` 并锁定执行顺序（测试先行，0.5 人日）
  - [ ] 4.1 先写会失败的顺序、clamp 与守卫用例
...
- [ ] 6. 备份导出与恢复携带 fork 水位（测试先行，0.75 人日）
  - [ ] 6.1 先写会失败的备份用例
