...
- [ ] 2. 前端基线清债（0.5 人日）
  ...
  - [ ] 2.2 修复 3 个因 `DOMMatrix` 崩溃的套件
    ...
    - 改动：在下面三个文件里，把 `import { describe, expect, it } from "vitest"` 改为同时导入 `vi`，并在导入块之后加 `vi.mock("react-pdf", () => ({ Document: () => null, Page: () => null, pdfjs: { GlobalWorkerOptions: { workerSrc: "" } } }))`。写法照抄 `src/components/DocumentPreviewCore.pdfSkeleton.test.tsx` ≈L5-9；`.ts` 文件不能写 JSX，所以组件返回 `null`。
...
- [ ] 4. Makefile 契约：`Makefile.intranet`（0.5 人日）
  - [ ] 4.1 先写合同测试的 Makefile 部分（此时应为红）
    ...
    - _需求：5.1, 5.4_
  - [ ] 4.2 前端目标
    - 改动：新增 `Makefile.intranet`，内容包括头注释、`NPM_REGISTRY ?=`、`install-frontend`（`npm ci --no-audit --no-fund`，`NPM_REGISTRY` 非空时追加 `--registry=…`）、`test-frontend`（`npm run test`）、`check-frontend: typecheck-frontend lint-frontend test-frontend`。在根 `Makefile` 末尾（`version` 目标之后，≈L342 后）追加一行 `include Makefile.intranet`，其余一行不动。
...
- [ ] 5. CI 调用方：`ci.yml` 两个新 job（0.5 人日）
  - [ ] 5.1 先补齐合同测试的 CI 部分（此时应为红）
    - 改动：在 `tests/unit/test_ci_gates_contract.py` 中新增以下测试：
      - `test_ci_has_frontend_job_calling_make_targets`：`frontend` job 的 run 步骤包含 `make install-frontend` 与 `make check-frontend`，并带 `sync-develop-after-` 条件。
      - `test_ci_has_postgresql_job_with_service_and_step_scoped_dsn`：`services.postgres.image` 以 `postgres:` 开头；DSN 只出现在执行 `make test-postgresql` 那一步的 `env` 里，不在 job 级 `env`。
      - `test_dsn_only_in_postgresql_job`：其他 job 序列化后不含 `OCTOP_TEST_DATABASE_URL`。
      - `test_backend_jobs_still_run_make_test`：`quality` 与 `test-windows` 仍执行 `make test`。
      - 用 `yaml.safe_load` 读取 `.github/workflows/ci.yml`。
    - 验证：`uv run pytest tests/unit/test_ci_gates_contract.py -q`（期望 CI 相关用例失败）
    - _需求：5.1, 5.2, 5.4_
  - [ ] 5.2 插入 `frontend` 与 `postgresql` job
